...
const ScheduleCalendar = ({ stylist, onClose }) => {
  ...
  // Helper functions for save operation
  const getDayOfWeek = (date) => {
    return new Date(date).getDay();
  };
  ...
  const getLatestTime = (slots) => {
    ...
    const max = Math.max(...times) + 15;
    const h = String(Math.floor(max / 60)).padStart(2, "0");
    const m = String(max % 60).padStart(2, "0");
    return `${h}:${m}`;
  };
  ...
  const handleSaveSchedule = async () => {
    ...
    try {
      ...
      if (scheduleType === "leave") {
        ...
        const dayOfWeek = getDayOfWeek(weekDates[dragStart.dayIndex]);
        const startTime = getEarliestTime(selectedTimeSlots);
        const endTime = getLatestTime(selectedTimeSlots);
        ...
        alert(`Schedule saved! Time range: ${startTime} - ${endTime} on ${["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"][dayOfWeek]}.`);
        setSelectedTimeSlots([]);
      }
