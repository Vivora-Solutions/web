...
const AddStylistModal = ({
  ...
  const handleProfilePicUpload = async (e) => {
    ...
    try {
      ...
      setFormData((prev) => ({
        ...prev,
        profile_pic_link: urlData.publicUrl,
      }));
  ...
  return (
    <>
      ...
      {/* Responsive Modal */}
      <div className="fixed right-0 top-0 h-screen w-full sm:w-[400px] bg-white shadow-2xl z-50 overflow-y-auto animate-slide-in flex flex-col">
        {/* Header */}
        <div className="p-4 sm:p-6 border-b-2 border-gray-200 bg-gradient-to-r from-indigo-500 to-purple-600 text-white flex justify-between items-center">
          <h2 className="text-lg sm:text-xl font-bold">Add New Employee</h2>
          <button
            onClick={handleClose}
            className="p-2 rounded-full bg-white/20 hover:bg-white/30 transition-all"
          >
        ...
        <div className="p-4 sm:p-6 flex-1">
          {/* Name */}
          <div className="mb-4">
            <label className="block font-semibold text-gray-700 mb-1 sm:mb-2 text-sm">
              Name
            </label>
            <input
              type="text"
              ...
              required
              className="w-full px-3 sm:px-4 py-2 sm:py-3 border-2 border-gray-200 rounded-lg text-sm transition-all focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
            />
          </div>
