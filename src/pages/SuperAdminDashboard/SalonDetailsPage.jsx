...
const SalonDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [salonRes, bookingRes] = await Promise.all([
          ProtectedAPI.get(`/super-admin/salons/${salonid}`),
          ProtectedAPI.get(`/super-admin/booking/${salonid}`)
        ]);
        setSalonInfo(salonRes.data);
                        ...
                        <React.Fragment key={booking.booking_id}>
                          {services.map((s, idx) => (
                            <tr key={s.booking_service_id}>
                              {idx === 0 && (
                                <td rowSpan={services.length} className="border px-4 py-2 align-top">
                                  {formattedDate}
                                </td>
                              )}
                              <td className="border px-4 py-2">{s.service?.service_name || 'N/A'}</td>
                              <td className="border px-4 py-2">{booking.stylist?.stylist_name || 'N/A'}</td>
                              <td className="border px-4 py-2">{s.service_duration_at_booking} mins</td>
                              <td className="border px-4 py-2">{formattedTime}</td>
                              <td className="border px-4 py-2">Rs. {s.service_price_at_booking}</td>
                              {idx === 0 && (
                                <td rowSpan={services.length} className="border px-4 py-2 align-top">
                                  {booking.booked_mode}
                                </td>
            ...
            <div className="md:hidden space-y-4">
              {filteredBookings.length > 0 ? (
                filteredBookings.map((booking) => {
                  ...
                  return (
                    <div key={booking.booking_id} className="bg-white p-4 rounded-xl shadow-md">
                      <p className="text-sm text-gray-500">{formattedDate}</p>
                      <p className="font-semibold text-gray-800">{booking.stylist?.stylist_name || 'N/A'}</p>
                      {booking.booking_services?.map((s) => (
                        <div key={s.booking_service_id} className="mt-2">
                          <p className="text-gray-700">{s.service?.service_name || 'N/A'}</p>
                          <p className="text-sm text-gray-500">{s.service_duration_at_booking} mins | Rs. {s.service_price_at_booking}</p>
                        </div>
                      ))}
                      <p className="text-sm text-gray-600 mt-2">{formattedTime}</p>
                      <p className="text-sm text-gray-600">Mode: {booking.booked_mode}</p>
